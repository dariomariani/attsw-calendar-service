...
public class EventServiceTest {
	...
	public void setup() {
		...
		standardUser1 = new User(id1, TestUserDataset.USERNAME1);
		standardUser2 = new User(id2, TestUserDataset.USERNAME2);
	...
	public void testCreateEventWhenEventNameIsNullShouldThrowIllegalArgumentException() {
		...
		assertThrows("EventName cannot be null nor blank.", IllegalArgumentException.class,
	...
	public void testCreateEventWhenStartDateIsNullShouldThrowIllegalArgumentException() {
		...
		assertThrows("StartsAt cannot be null.", IllegalArgumentException.class,
				() -> eventService.createEvent(newEvent));
	}
	...
	public void testCreateEventWhenEndsAtIsPriorStartsAtThrowIllegalArgumentException() {
		var newEvent = new Event("Event with negative timespan", standardUser1, LocalDateTime.now(),
				LocalDateTime.now().minusHours(2));
		assertThrows("EndsAt must be after StartsAt.", IllegalArgumentException.class,
				() -> eventService.createEvent(newEvent));
	}
	...
	public void testCreateEventWhenEndDateIsNullShouldThrowIllegalArgumentException() {
		...
		assertThrows("EndsAt cannot be null.", IllegalArgumentException.class,
				() -> eventService.createEvent(newEvent));
	}
	...
	public void testFindEventById() {
		...
		Event expectedEvent = new Event("Event 1", standardUser1, LocalDateTime.now().plusHours(1),
				LocalDateTime.now());
	...
	public void testUpdateEvent() {
		// create a test event
		Event event = new Event("Test Event", standardUser1, LocalDateTime.of(2023, 2, 22, 10, 0),
				LocalDateTime.of(2023, 2, 22, 11, 0));
		// update the test event
		Event updatedEvent = new Event("Updated Event", standardUser2, LocalDateTime.of(2023, 2, 23, 11, 0),
				LocalDateTime.of(2023, 2, 23, 12, 0));
	...
	public void testUpdateInvalidEventThrowsException() {
		UUID eventId = UUID.randomUUID();
		// create a test event
		Event event = new Event(eventId, "Test Event", standardUser1, LocalDateTime.of(2023, 2, 22, 10, 0),
				LocalDateTime.of(2023, 2, 22, 11, 0));
		// existing event
		Event existingEvent = new Event("Existing Event", standardUser1, LocalDateTime.of(2023, 2, 23, 10, 0),
				LocalDateTime.of(2023, 2, 23, 11, 0));
		// create a test event
		Event updatedEvent = new Event(eventId, "Test Event", standardUser1, LocalDateTime.of(2023, 2, 23, 10, 0),
				LocalDateTime.of(2023, 2, 23, 11, 0));
		// add the test event to the repository
		when(eventRepository.findById(event.getId())).thenReturn(event);
		when(eventRepository.findAll()).thenReturn(Arrays.asList(existingEvent));
		assertThrows("Event not found", IllegalArgumentException.class, () -> {
			eventService.updateEvent(updatedEvent);
		});
	}
	...
	public void testFindEventByUserAndPeriodWithNoOverlap() {
		...
		List<Event> events = eventService.findEventByUserAndPeriod(user1, LocalDateTime.of(2023, 2, 22, 12, 0, 0),
	...
	public void testFindEventByUserAndPeriodWithOverlap() {
		...
		EventService eventService = new EventService(eventRepository);
		List<Event> events = eventService.findEventByUserAndPeriod(user1, LocalDateTime.of(2023, 2, 21, 13, 0, 0),
				LocalDateTime.of(2023, 2, 22, 13, 0, 0));
